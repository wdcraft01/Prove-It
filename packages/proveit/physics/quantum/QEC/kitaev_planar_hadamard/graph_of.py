...
class MergedBoundsGraphOfSiteOp(Function):
    ...
    merged-bounds graph of surface code S, corresponding to site
    operator s of surface code S. See above for some description of
    the MergedBoundsGraphOfSurfaceCode(S). This is really just a
    complicated way to refer to the graph vertex v that corresponds
    to a specific site operator s in the overall surface code S.
    Notice that MergedBoundsGraphOfSiteOp(s, S) is not a set but
    instead an element of Vertices(MergedBoundsGraphOfSurfaceCode(S)).
    '''

    # the literal operator for the MergedBoundsGraphOfSiteOp() operation
    _operator_ = Literal(
            string_format='mb_graph_so',
            # latex_format=r'\mathrm{mb\_graph}_{\mathit{zerr}}\!',
            latex_format=r'{{\mathcal{G}}_{mb_{so}}{\!}}',
            theory=__file__)
